...
mongoose.Model.handle = function(options){
	...
	return function(req, res){
		//var context = {req:req, user:req.user, scope:baseScope||scope, overscope:baseScope?scope:null, path:""};
		var filter = {};
		...
		if(splitPath.length == 0){
			var args = req.mixin.clone().merge({user:req.user, callback:res.handle()});
			model.process._apply(me, args);
		}
		else{
			var collDoc = model;
			var popCollDoc = function(callback){
				if(collDoc){
				...
				if(pathPart == urlPart){
					...
					if(caller instanceof mongoose.Document && urlPart in caller.getModel().schema.tree && !caller.populated(urlPart)){
			...
			popCollDoc(function(err, collDoc){
				if(!err){
